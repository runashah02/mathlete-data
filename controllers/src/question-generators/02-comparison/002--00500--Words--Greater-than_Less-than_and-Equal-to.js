...
let exerciseDesc = `
Which words make the statement true? A ___ B
3 options are given: is less than, is equal to, is greater than
To solve, select the correct option.

This exercise generates 3 variations: 1. is less than, 2. is equal to, 3. is greater than
...
function generateQuestionPart0(numbers, slab, signCodes, answerCode) {
  let qstmt = `<p><span class='audio-enabled'>Which words make the statement true?</span></p>`;
  return qstmt;
}

function generateQuestionPart1(numbers, slab, signCodes, answerCode) {
  let qstmt = `<div class="display-grid">`;
  qstmt += `<div>${util.numberToHTML(numbers[0])}</div>`;
  qstmt += `<div answer-code="${answerCode}" class="ca-blank"></div>`;
  qstmt += `<div>${util.numberToHTML(numbers[1])}</div>`;
  qstmt += `</div>`;
  return qstmt;
}
...
function generateAnswerKeys2(numbers, slab, signCodes, answerCode) {
  ...
  let answerKey = {
    ...
    clickTimes: 1,
    content: `<span class='insert-content'>is less than</span>`,
  };
  answerKeys.push(answerKey);
  answerKey = {
    answerCodes: [signCodes[1]],
    clickTimes: 1,
    content: `<span class='insert-content'>is equal to</span>`,
  };
  answerKeys.push(answerKey);
  answerKey = {
    answerCodes: [signCodes[2]],
    clickTimes: 1,
    content: `<span class='insert-content'>is greater than</span>`,
  };
  answerKeys.push(answerKey);
  return answerKeys;
}
...
function generateAnswerExplanation(numbers, slab, signCodes, answerCode) {
  ...
  if (numbers[0] == numbers[1]) {
    ...
    expln += `<p><span class="audio-enabled">So, ${util.numberToHTML (numbers[0])} <span class="em-answer">is equal to</span> ${util.numberToHTML (numbers[1])}</span>.</p>`;
    return expln;
  }
...
function compareByCounting(numbers, slab, signCodes, answerCode) {
  ...
  if (numbers[0] < numbers[1]) {
    ...
    expln += `<p><span class="audio-enabled">So, ${util.numberToHTML (numbers[0])} <span class="em-answer">is less than</span> ${util.numberToHTML (numbers[1])}</span>.</p>`;
  } else {
    expln += `<p><span class="audio-enabled">${util.numberToHTML (numbers[0])} comes after ${util.numberToHTML (numbers[1])}</span>.</p>`;
    expln += `<p><span class="audio-enabled">So, ${util.numberToHTML (numbers[0])} <span class="em-answer">is greater than</span> ${util.numberToHTML (numbers[1])}</span>.</p>`;
  }
  return expln;
}
...
function compareWithPlaceChart(numbers, slab, signCodes, answerCode) {
  ...
  if (numbers[0].toString().length > numbers[1].toString().length) {
    ...
    expln += `${util.numberToHTML (numbers[0])} is greater than ${util.numberToHTML (numbers[1])}`;
    ...
    expln += `${util.numberToHTML (numbers[0])} is less than ${util.numberToHTML (numbers[1])}`;
  } else {
...
function generateQuestion(questionNo, numbers, slab) {
  ...
  return {
    ...
    moduleId: 2,
    numberRangeMin: slab.min1,
    numberRangeMax: slab.max2,
