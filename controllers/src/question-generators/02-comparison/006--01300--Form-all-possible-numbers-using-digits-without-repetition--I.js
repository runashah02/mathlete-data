// This program generates questions that ask students to form the smallest or largest number
// from a given set of digits.
// Question Type: "Form the smallest/largest number"
// Range: 10 to 999 for 2 and 3-digit numbers, and randomly chosen 4-digit numbers from 1000-9999
//
// Topic: Comparing and Ordering Numbers
// Learning goal: Understand and practice formin1g numbers by arranging digits in ascending and descending order.
//
...
Form the numbers and fill in the blank with a list of numbers separated by a space
...
function generateQuestionStatement(digits) {
  const digitsString = digits
    ...
    .join(", ");
  return `<p><span class='audio-enabled'>Using the digits ${digitsString} only once, make all possible ${digits.length}-digit numbers. Then write the numbers in the box separating them with a space</span>.</p>`;
}
...
function generateAnswerExplanation(digits, possibleNumbers) {
  ...
  Using the digits ${digitStr} only once, it is possible to make ${possibleNumbers.length} numbers: <span class="em-answer">${possibleNumbers}</span></span></p>`;  
  return expln;
}
...
function generateQuestion(questionNo, digits, twistiness, slab) {
  ...
  const answerCodes = Array.from({ length: possibleAnswers.length }, util.generateRandom4);
  const allowedAnswers = [];
  for (let i = 0; i < possibleAnswers.length; ++i) {
    allowedAnswers.push([possibleAnswers[i]]);
  }
