...
function generateQuestionPart0(number) {
  let qstmt = ``;
  qstmt += `<p><span class='audio-enabled'>Write the number for <span style='font-weight:bold'>${util.generateNumberName (number, "International")}</span><span></p>`;
  return qstmt;
}
...
function generateAnswerExplanation(number) {
  ...
  if (number > 99) {
    ...
    answerExpln += util.placeValueChart(number, "International");
    answerExpln += `</br>`;
  }
        ...
        answerKeys: [
          {
            ...
            allowedAnswers: [[[util.addCommasToNumber (number, "International")]]],
          },
        ],
      },
    ],
