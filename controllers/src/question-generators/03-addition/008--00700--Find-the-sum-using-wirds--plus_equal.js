...
function generateQuestionStatement(numbers, answerCode) {
  let qstmt = "";
  qstmt += `<div style='display:flex;align-items:center'><span style="margin-right:20px;white-space:nowrap">${numbers[0]} plus ${numbers[1]} is equal to </span><input type='text' name='${answerCode}' answer-code='${answerCode}' size='10' class='form-control blank'> </div>`;
  qstmt += `</div>`;
  return qstmt;
}
...
function generateAnswerExplanation(numbers, icons) {
  ...
  answerExplanation += `<p class='audio-enabled'>Count the ${objectNames}, like this:</p>
                          ...
                          so ${numbers[0]} plus ${numbers[1]} is equal to ${sum}</p>`;
