// Solve A + B = C, where A and B is a group of countable objects of the same kind.
// The student must provide the sum in the blank

// To be asked for Addition of numbers (sums upto 5, 10 and 18)
// Learning goal: Understand the concept of A + B = C using visual illustration
...
function generateQuestionStatement(numbers, icons, answerCode) {
    ...
    qstmt += "<div> = </div>";
    qstmt += `<div><input type='text' name='${answerCode}' answer-code='${answerCode}' size='2' class='form-control blank'></div>`;
    qstmt += `</div>`;
    return qstmt;
  }
...
function generateAnswerExplanation(numbers, icons) {
  ...
  return `<p class='audio-enabled'>To add, count the ${objectNames}, like this:</p>
          ...
          <p></p>
          <p class='audio-enabled'>There are ${sum} ${objectNames}.</p>`;
}
...
function createQuestion(questionNo, numbers, icons, topicId, exerciseId) {
  ...
  let exerciseCustomCSS = `.equation-container {display: grid; grid-template-columns: repeat(5, auto); gap:10px; width: fit-content; align-items: center }
