...
function generateQuestionStatement (numbers,iconPair) {
    let qstmt = "";
    qstmt += `<p>Which addition sentence correctly represents the picture</p>${util.generateJointSentence(iconPair, numbers, false, false)}`;
    return qstmt;
  }
  ...
  function generateAnswerExplanation (numbers, iconPair){
    ...
    <p><div class='equation-container'>${util.generateObjectsContainerPartial(iconPair[0].iconName, numbers[0], "left", true, 1)}</div></p>
    ...
    <p>${util.generateJointSentence (iconPair, numbers, false, false)}</p>`;
...
function createQuestion(questionNo, numbers, iconPair, topicId, exerciseId) {
    ...
    let questionStatement = generateQuestionStatement(numbers, iconPair);
  ...
  return {
    ...
    parts: [
      ...
      },
      {
        ...
        answerKeys: [
          {
            content: `${uniqueOptions[0][0]}+${uniqueOptions[0][1]}=${uniqueOptions[0][0] + uniqueOptions[0][1]}`,
            points: 0.25,
            answerCodes: [generateRandom4()],
            allowedAnswers: [[[uniqueOptions[0][0] == numbers[0] && uniqueOptions[0][1] == numbers[1] ? 1 : 0]]],
          },
          {
            content: `${uniqueOptions[1][0]}+${uniqueOptions[1][1]}=${uniqueOptions[1][0] + uniqueOptions[1][1]}`,
            points: 0.25,
            answerCodes: [generateRandom4()],
            allowedAnswers: [[[uniqueOptions[1][0] == numbers[0] && uniqueOptions[1][1] == numbers[1] ? 1 : 0]]],
          },
          {
            content: `${uniqueOptions[2][0]}+${uniqueOptions[2][1]}=${uniqueOptions[2][0] + uniqueOptions[2][1]}`,
            points: 0.25,
            answerCodes: [generateRandom4()],
            allowedAnswers: [[[uniqueOptions[2][0] == numbers[0] && uniqueOptions[2][1] == numbers[1] ? 1 : 0]]],
          },
          {
            content: `${uniqueOptions[3][0]}+${uniqueOptions[3][1]}=${uniqueOptions[3][0] + uniqueOptions[3][1]}`,
            points: 0.25,
            answerCodes: [generateRandom4()],
