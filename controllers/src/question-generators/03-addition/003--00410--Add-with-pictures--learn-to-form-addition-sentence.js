...
function generateAnswerExplanation(numbers, icons) {
  ...
  return `<p class='audio-enabled'>To add, count the ${objectNames}, like this:</p>
          ...
          so ${numbers.join(" + ").replace(/, ([^,]*)$/, " and $1") + " = "}${sum}</p>`;
}
...
function generateQuestionStatement(numbers, icons, answerCode) {
  ...
  for (let i = 0; i < numbers.length; ++i) {
    qstmt += `<div>${numbers[i]}`;
    qstmt += `</div>`;
    qstmt += i < numbers.length - 1 ? `<div> + </div>` : "";
  }

  // The word "make" and input must be in the same row. Therefore, setting display to flex
  qstmt += `<div style='display:flex;align-items:center'><span style="margin-right:20px;"> = </span><input type='text' name='${answerCode}' answer-code='${answerCode}' size='2' class='form-control blank'> </div>`;
  qstmt += `</div>`;
  return qstmt;
}
