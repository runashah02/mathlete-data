...
function generateQuestionStatement(numbers) {
  let qstmt = "";
  qstmt += `<p class='audio-enabled'>Write this as an addition sentence using + and = </p>`;
  qstmt += `<div>${numbers[0]} and ${numbers[1]} make ${numbers[0] + numbers[1]}</div>`;
  return qstmt;
}
...
function createQuestion(questionNo, numbers, icons, topicId, exerciseId) {
  ...
  let exerciseCustomCSS = `.equation-container {display: grid; grid-template-columns: repeat(4, auto); gap:10px; width: fit-content;  grid-auto-rows: 50px; align-items: center }
